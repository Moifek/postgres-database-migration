...
if [[ $(ls logs | grep "$(date +"%Y-%m-%d")") ]]; then
	echo "log repos already created"
else
	mkdir "logs/$(date +"%Y-%m-%d")"
	mkdir "logs/mongo/$(date +"%Y-%m-%d")"
	mkdir "logs/postgres/$(date +"%Y-%m-%d")"
fi
...
if [ -z "$postgresTestDatabase_host" ] || [ -z "$postgresTestDatabase_port" ] || \
   ...
   [ -z "$MongoDatabase_password" ] || [ -z "$MongoDatabase_collection" ]; then
    echo "[ERROR][$(date +"%Y-%m-%d %T")] Required database variables are not set in $INI_FILE" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
    exit 1
fi
# Check if the pagination variables are set
if [ -z "$pagination_pageSize" ] || [ -z "$pagination_pageNumber" ]; then
        echo "[ERROR][$(date +"%Y-%m-%d %T")] Required pagination variables are not set in $INI_FILE" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
        exit 1
fi
...
if [ -z "$databasePassword" ]; then
        echo "[ERROR][$(date +"%Y-%m-%d %T")] database secrets not fetched correctly" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
        exit 1
else
        echo "[INFO][$(date +"%Y-%m-%d %T")] database secrets fetched correctly" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
fi
...
execute_sql() {
    ...
    local error_file="$(date +"%Y-%m-%d")/error-page${page_number}.log"
...
#execute the psql procedure
    ...
    if [[ $psql_exit_status -ne 0 ]]; then
        ...
        echo "[ERROR][$(date +"%Y-%m-%d %T")] Failed to execute SQL for page number $page_number. Check $error_file for details." | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
        return 1
    fi
    ...
    if [[ $psql_exit_status -eq 0 && $(cat $output_file | wc -c) -gt 1 ]]; then
        ...
        echo "[INFO][$(date +"%Y-%m-%d %T")] No more data to export" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
...
import_to_mongo() {
    local page_number=$1
    #shellcheck disable=SC2155
    local error_file="$(date +"%Y-%m-%d")/error-on-Iteration${page_number}.log"
    local temp_error_file="temp_error.log"
	#Iterator doesn't really have a special functionality here
        echo "[INFO][$(date +"%Y-%m-%d %T")] using file exported-images-page$Iterator.json" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
        mongoimport --host "$MONGO_DB_HOST":"$MONGO_DB_PORT" --db "$MONGO_DB_NAME" --collection "$MONGO_DB_COLLECTION" --numInsertionWorkers=4 \
	--type json --file exported-images-page"$Iterator".json --jsonArray -vvv --stopOnError  2> "logs/mongo/$temp_error_file"
	#--mode upsert --upsertFields uuid
	echo "iteration : $Iterator" >> "logs/mongo/execution.log"
	cat "logs/mongo/$temp_error_file" >> "logs/mongo/execution.log"
	local mongo_exit_code=$?

	if [[ $( < "logs/mongo/$temp_error_file" grep "Failed:" | wc -c) -ge 1 ]]; then
		< "logs/mongo/$temp_error_file" tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
		mv "logs/mongo/$temp_error_file" "logs/mongo/$error_file"
		echo -e "[ERROR][$(date +"%Y-%m-%d %T")]  import failed for file exported-images-page$Iterator.json\n -> consult /logs/mongo/$error_file" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
		return 1
	elif [[ $( < "logs/mongo/$temp_error_file" grep "failed to connect" | wc -c) -ge 1 ]]; then
		< "logs/mongo/$temp_error_file" tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
                mv "logs/mongo/$temp_error_file" "logs/mongo/$error_file"
                echo -e "[ERROR][$(date +"%Y-%m-%d %T")] Connection failed to mongo database \n -> consult /logs/mongo/$error_file" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
                return 1
	else
		echo -e "[INFO][$(date +"%Y-%m-%d %T")] import succeeded for file exported-images-page$Iterator.json\n" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
	fi
	Iterator=$((Iterator + 1))
...
echo "[INFO][$(date +"%Y-%m-%d %T")] Schema set to $DATABASE_SCHEMA" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
...
if [[ $? -ne 0  || $( < "logs/init_errors.log" tr ' ' _ | nl ) -ne 0 ]]; then
    echo "[ERROR][$(date +"%Y-%m-%d %T")] Failed to execute the initial SQL script. consult logs/init_erros.log" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
    exit 1
fi

echo "[INFO][$(date +"%Y-%m-%d %T")] Procedure created on $DATABASE_NAME - ready to extract" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"

# Loop to handle pagination
while true; do
        echo -e "[INFO][$(date +"%Y-%m-%d %T")] executing extraction \n - iteration : $PAGE_NUMBER \n - batch size : $PAGE_SIZE " | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
    execute_sql "$PAGE_NUMBER"
    exit_code=$?
    if [[ $exit_code -eq 1 ]]; then
        update_ini_file "$INI_FILE" "$LAST_SUCCESSFUL_ITERATION"
	echo -e "[ERROR][$(date +"%Y-%m-%d %T")] Script ended with error and Variables reset to last successful iteration number $LAST_SUCCESSFUL_ITERATION.\n" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
        exit 1
    fi
    if [[ $exit_code -eq 2 ]]; then
        echo "[INFO][$(date +"%Y-%m-%d %T")] Script ended and data exported with $((PAGE_NUMBER - 1)) total pages, starting with $LAST_SUCCESSFUL_ITERATION" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
        exit 2
    fi
	...
	if [[ $mongo_exit_code -eq 1 ]]; then
		echo "[ERROR][$(date +"%Y-%m-%d %T")] an error occured during data import on mongo database" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
		echo "[ERROR][$(date +"%Y-%m-%d %T")] Script ended with error and Variables reset to last successful iteration $LAST_SUCCESSFUL_ITERATION ." | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
		exit 1
	elif [[ $mongo_exit_code -eq 0 ]]; then
		echo -e "[INFO][$(date +"%Y-%m-%d %T")] imported successfully\n" | tee -a "logs/$(date +"%Y-%m-%d")/execution.log"
